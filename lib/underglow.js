...
export class UnderglowManager {
    ...
    enable() {
        ...
        this._signals.push(this._settings.connect('changed::underglow', () => {
            this._syncEnabledFromSettings();
        }));
        this._signals.push(this._settings.connect('changed::neon-color', () => {
            this._refreshColor();
            this._restyleAll();
        }));
        this._signals.push(this._settings.connect('changed::music-reactive', () => {
    ...
    setAudioIntensity(mult, beatPulse = 0) {
        ...
        if (this._enabled && this._musicReactive)
            this._restyleAll();
    }
    ...
    _connectThemeSignals() {
        if (_themeSettingsSignals.length > 0)
            return;
        ...
        _themeSettingsSignals.push(settings.connect('changed::gtk-theme', onThemeChanged));
        if (settings.list_keys().includes('gtk4-theme')) {
            _themeSettingsSignals.push(
                settings.connect('changed::gtk4-theme', onThemeChanged)
            );
        }
    }
    ...
    _disconnectThemeSignals() {
        ...
        const settings = themeSettings();
        for (const id of _themeSettingsSignals)
            settings.disconnect(id);
        _themeSettingsSignals = [];
    }
    ...
    _syncEnabledFromSettings() {
        ...
        const on = this._settings.get_boolean('underglow');

        if (on) {
            if (!this._globalConnected) {
                this._connectGlobalSignals();
                this._globalConnected = true;
            }
    ...
    _connectGlobalSignals() {
        this._displaySignals = [
            global.display.connect('window-created', (_d, metaWindow) => {
                if (!this._enabled)
                    return;
        ...
        this._wmSignals = [
            getWindowManager().connect('destroy', (_wm, actor) => {
                this._removeUnderglow(actor);
            }),
        ];
    }
    ...
    _disconnectGlobalSignals() {
        ...
        for (const id of this._wmSignals ?? [])
            getWindowManager().disconnect(id);
        this._displaySignals = [];
        this._wmSignals = [];
    }
    ...
    _ensureUnderglow(windowActor) {
        if (!this._enabled || !actorAlive(windowActor))
            return;
    ...
    _createUnderglow(windowActor, win) {
        ...
        const onWindowChanged = () => {
            if (!this._enabled || !this._entries.has(windowActor))
                return;
            this._updateShadowStyle(windowActor);
        };
        entry.winSignals.push(win.connect('notify::appears-focused', onWindowChanged));
        entry.winSignals.push(win.connect('notify::maximized-horizontally', onWindowChanged));
        entry.winSignals.push(win.connect('notify::maximized-vertically', onWindowChanged));
        entry.winSignals.push(win.connect('notify::fullscreen', onWindowChanged));
        entry.winSignals.push(win.connect('size-changed', onWindowChanged));
        entry.winSignals.push(win.connect('position-changed', onWindowChanged));
    ...
    _removeUnderglow(windowActor) {
        ...
        try {
            if (actorAlive(windowActor)) {
                const win = windowActor.get_meta_window?.() ?? windowActor.meta_window;
                if (win) {
                    for (const id of entry.winSignals ?? [])
                        win.disconnect(id);
                }
            }
        } catch {
