...
export const AudioVisualizer = GObject.registerClass({
    GTypeName: 'CyberGlowAudioVisualizer',
}, class AudioVisualizer extends GObject.Object {
    _init() {
        super._init();
    ...
    _ensureGst() {
        if (this._gstInitialized || Gst.is_initialized()) {
    ...
    _defaultMonitorDevice() {
        if (this._mixer) {
            try {
                ...
                // Fall through to the D-Bus fallback.
            }
        }
    ...
    _stopPipeline() {
        ...
        const bus = this._pipelineBus ?? this._pipeline.get_bus();
        if (bus) {
            bus.remove_signal_watch();
            bus.disconnectObject(this);
        }
    ...
    _readSpectrumMagnitudes(structure) {
        ...
        if (ok && magnitudes && magnitudes.n_values > 0)
            return magnitudes;
